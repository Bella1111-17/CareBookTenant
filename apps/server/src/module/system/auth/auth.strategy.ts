...
import { PLATFORM_USER_SCOPE } from 'src/common/tenant/tenant.constants';
...
export class AuthStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(payload: { uuid: string; userId: string; iat: Date }) {
    ...
    this.tenantContextService.setTenantContext({
      tenantId: user.tenantId ?? user.user?.tenantId ?? null,
      userId: user.userId ?? user.user?.userId ?? null,
      userScope: user.userScope ?? user.user?.userScope ?? PLATFORM_USER_SCOPE,
    });
    return user;
  }
}
