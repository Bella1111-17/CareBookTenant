...
export class RoleService {
  ...
  async create(createRoleDto: CreateRoleDto) {
    const res = await this.sysRoleEntityRep.save({
      ...createRoleDto,
      tenantId: this.resolveWriteTenantId((createRoleDto as any).tenantId),
    });
  ...
  async findAll(query: ListRoleDto) {
    ...
    applyTenantScope(entity, 'entity', this.tenantContextService, { requestedTenantId: (query as any).tenantId });
  ...
  async findOne(roleId: number) {
    const res = await this.sysRoleEntityRep.findOne({
      where: {
        roleId: roleId,
        delFlag: '0',
        tenantId: this.tenantContextService.isPlatformUser() ? undefined : this.tenantContextService.getTenantId(),
      },
    });
    return ResultData.ok(res);
  }
   ...
   * 根据角色获取用户权限列表
   */
  async getPermissionsByRoleIds(roleIds: number[]): Promise<{ perms: string }[]> {
