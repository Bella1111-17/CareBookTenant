...
/**
 * 护理业务时间统一按东八区解析。
 * 对 YYYY-MM-DD HH:mm:ss 这类无时区字符串，明确视为 Asia/Shanghai 本地业务时间。
 */
export function ParseBusinessDateTime(value?: string | Date | null): Date | null {
  if (!value) {
    return null;
  }
  ...
  const parsed = dayjs.tz(raw.replace('T', ' '), 'YYYY-MM-DD HH:mm:ss', 'Asia/Shanghai');
  if (parsed.isValid()) {
    return parsed.toDate();
  }

  const fallback = dayjs.tz(raw, 'Asia/Shanghai');
