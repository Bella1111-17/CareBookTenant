    ...
    <div class="app-container tenant-device-page">
      ...
      <el-tab-pane label="设备管理" name="device">
        <el-form ref="deviceQueryRef" :model="deviceQuery" :inline="true" v-show="showSearch" class="toolbar-form">
          ...
          <el-form-item>
            <el-button v-if="!isPlatformUser" type="primary" plain icon="Plus" @click="openDeviceDialog()" v-hasPermi="['tenant-care:badge:add']">新增设备</el-button>
          </el-form-item>
        ...
        <el-table v-loading="deviceLoading" :data="deviceList">
          ...
          <el-table-column label="归属时间" width="165" align="center">
            <template #default="{ row }">{{ row.createTime ? parseTime(row.createTime) : '-' }}</template>
          </el-table-column>
          ...
          <el-table-column label="操作" :width="isPlatformUser ? 100 : 300" align="center" fixed="right">
            <template #default="{ row }">
              <el-button v-if="!isPlatformUser" link type="primary" icon="EditPen" @click="openDeviceDialog(row)" v-hasPermi="['tenant-care:badge:edit']">编辑</el-button>
              <el-button v-if="!isPlatformUser" link type="primary" icon="Switch" @click="openBindDialog(row)" v-hasPermi="['tenant-care:badge:bind']">
                {{ row.bindingStatus === 'BOUND' ? '换绑' : '绑定' }}
              </el-button>
              <el-button v-if="!isPlatformUser && row.bindingStatus === 'BOUND'" link type="danger" icon="Remove" @click="openUnbindDialog(row)" v-hasPermi="['tenant-care:badge:unbind']">解绑</el-button>
              <el-button v-else-if="!isPlatformUser" link type="danger" icon="Delete" @click="handleDeleteDevice(row)" v-hasPermi="['tenant-care:badge:remove']">删除</el-button>
...
function handleDeleteDevice(row) {
  ElMessageBox.confirm(`确认删除设备 ${row.deviceNo} 吗？删除后设备主档将被移除。`, '删除设备确认', {
    confirmButtonText: '最终确认删除',
    cancelButtonText: '取消',
    type: 'warning',
...
function flowItemType(eventType) {
  if (eventType === 'CAREGIVER_BOUND') return 'success'
  if (eventType === 'CAREGIVER_UNBOUND') return 'warning'
  return 'info'
}
