...
service.interceptors.response.use(res => {
    ...
    if (code === 401) {
      ...
      if (!originalRequest.url?.includes('/refresh-token') && !originalRequest._retry && refreshTokenValue) {
        ...
        return refreshTokenApi(refreshTokenValue).then((data) => {
          const nextToken = data.data?.accessToken || data.accessToken
          ...
          userStore.refreshToken = nextRefreshToken || userStore.refreshToken
